...
window.AppController = {
    ...
    // ==========================================
    // UI EVENTS (Delegated to body for safety)
    // ==========================================
    if (!window.mapListenerBound) {
        document.body.addEventListener('click', (e) => {
            if (e.target && e.target.id === 'btnRevealMap') {
                const btnRevealMap = e.target;
                const mapWrapper = document.getElementById('mapWrapper');
                const locDetails = document.getElementById('locDetailsWrapper');
                
                if (mapWrapper && locDetails) {
        ...
        try {
            await fetch('http://localhost:3000/api/analysis', {
                ...
                body: JSON.stringify({
                    ...
                    aiProbability: result.checks.api ? result.checks.api.probability : 0
                })
            });
            if (window.app && window.app.fetchDashboardStats) {
                window.app.fetchDashboardStats();
            }
        } catch (err) {
    ...
    function renderLocationMap(result) {
        ...
        if (locationData.hasCoordinates && locationData.latitude !== null && locationData.longitude !== null) {
            ...
            document.getElementById('locDetailsWrapper').style.display = 'none';
            
            const btnRevealMap = document.getElementById('btnRevealMap');
            if (btnRevealMap) btnRevealMap.textContent = 'Reveal Map';
            ...
            setTimeout(() => {
                try {
                    window.leafletMap = L.map('locationMap', {
                        center: [lat, lon],
                        zoom: 14,
                        zoomControl: true,
                    ...
                    const marker = L.marker([lat, lon], { icon: markerIcon }).addTo(window.leafletMap);
    ...
    function downloadReport(result) {
        ...
        y += 10;
        doc.setFontSize(14);
        const vText = result.verdict.level === 'real' ? 'LIKELY HUMAN BUILT' : 
